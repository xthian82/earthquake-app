...
*/
    private static final String JSON_FEATURE = "features";
    private static final String JSON_PROPERTIES = "properties";
    private static final String JSON_MAG = "mag";
    ...
    public static URL createUrl(String urlReq) {
        URL url = null;
        try {
            url = new URL(urlReq);
     ...
     */
    public static List<Earthquake> parseEarthquakeListFromString(String jsonResponse) {
        ...
        try {
            ...
            JSONArray featArrays = response.getJSONArray(JSON_FEATURE);

            for (int i=0; i<featArrays.length(); i++) {
                Earthquake quake = new Earthquake();

                JSONObject item = featArrays.getJSONObject(i);
                JSONObject props = item.optJSONObject(JSON_PROPERTIES);

                if (props.has(JSON_MAG))   quake.setMagnitude         ( props.getDouble(JSON_MAG) );
                if (props.has(JSON_PLACE)) quake.setPlace             ( props.getString(JSON_PLACE) );
                if (props.has(JSON_TIME))  quake.setTimeInMilliseconds( props.getLong(JSON_TIME) );
                if (props.has(JSON_URL))   quake.setUrl               ( props.getString(JSON_URL) );
